/**
 * 映射函数测试脚本（简化版，使用CommonJS）
 * 测试mapOldToNew和mapNewToOld函数的正确性
 */

function generateId() {
  return 'entity-' + Math.random().toString(36).substring(2, 15);
}
...
function mapOldToNew(oldData) {
  ...
  return {
    entityId: oldData.uniqueId || generateId(),
...
function mapNewToOld(newEntity) {
  let metadata = {};
...
testCases.forEach((testCase, index) => {
  ...
  if (allMatch) {
    ...
    Object.entries(roundTripMatches).forEach(([key, match]) => {
      if (!match) {
        console.log(`  - ${key}: expected "${testCase.oldData[key]}", got "${oldData[key]}"`);
      }
    });
    failedTests++;
  }
